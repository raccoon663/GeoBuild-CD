# Model Provenance — SegFormer-B5 建筑分割模型来源核查
...
- 核查日期：2026-08-18
- 核查方式：磁盘上的训练配置、mmengine 日志、checkpoint 哈希、瓦片命名/栅格 transform 对账、ONNX 逐像素验证
- 结论：**原 SegFormer-B5 的主要人工监督是 April 建筑标注**；January 标签是独立人工标注，属于 independent temporal evaluation data，不是训练参考月。
...
### 3.1 `data/buildings`（原始训练集，April）
...
- 目录：`<LEGACY_PROJECT>/data/buildings`（备份：`<LEGACY_PROJECT>/data/buildings`）

### 3.2 `data/buildings_clean_eval`（干净评估集，April）
...
### 3.3 `data/buildings_hard_train`（hard negatives）
...
### 3.4 `shadow_b5_labelled_20260624`（shadow samples）
...
- GeoBuild-CD 副本：`checkpoints\segformer_b5_zhelin_local_best_20260624.pth`
...
- 旧 `scripts/verify_torch_vs_onnx.py` 报告的 ~0.7% mask flip 是 **iter-6000 vs iter-14000 两个模型的真实差异**，不是数值误差；该脚本的"数值差异"解释需要纠正。
...
- 哈希/验证脚本：`scripts/verify_torch_vs_onnx.py`（需按第 5 节结论重新解释）；本次核查脚本临时存放于本机 temp。
